...
* Description : 颜色拾取器V0.7.69 Beta
*               - 1.此版本暂不包含颜色模式的切换只有RGB模式，HSV模式预留
*               - 2.这个版本是初版，基本功能全部实现，由于没有预料到代码的复杂度，代码较长，耦合过度
*               * 此组件暂时不支持自适应，为固定大小组件，建议别修改
*               * RGB模式下六种调色板模式均支持
*               * 如果有什么bug请联系我，我将会一一处理 Emal : springu3d@yeah.net
==========================================*/
...
namespace SpringGUI
{
    public class ColorPicker : UIBehaviour
    {
        ...
        [SerializeField]
        private ColorPickerEvent m_evnet = new ColorPickerEvent();
